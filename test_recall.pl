# Z:\Stage\DC\U38150365a63bd7da6650x\result\547131650001.pdf
# D:\Stage\DC\U38150365a63bd7da6650x\result\544224100008.pdf
use warnings;
use strict;
use XML::Simple;
use Data::Dumper;
...
my $baseDir = 'D:\Stage\DC';
print "Base Directory : $baseDir";
# taking commad line argument for full path of pdf

my $pdfPath = $ARGV[0];       #Z:\Stage\DC\U38150113163c0e59f3c1x\result\546529103801.pdf
...
#reading the xml file to create part no. rev sequence
   
# my $file = "U381505d3063bfe86517xx.xml";

# my $test_data = XMLin($file);
my $test_data = XMLin($xmlfile);
...
#Soap Service Calling

# Create a new user agent object
my $ua = LWP::UserAgent->new;

# Create the XML document to post

# my $attrifile='recall_attri.txt';
# create a formatted XML
my $xml = sprintf(' <soapenv:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:get_recall_attributes">
   <soapenv:Header/>
   <soapenv:Body>
...
</soapenv:Envelope> ', $item, $rev, $seq);


# Create a new POST request with the XML document as the request body
my $req = HTTP::Request->new(POST => 'http://172.22.97.12:9080/cgi/get_recall_attributes.cgi');
...
#Response XML Parsing & Exporting in Text

my $test_data = XMLin($res->content);
my $final_res = $test_data->{'SOAP-ENV:Body'}->{'ns:GetRecallAttributesResponse'}->{'Response'};
my @s = split("~","$final_res");	
		open(FH, '>', "$resultDir\\attributes.txt") or die $!;
		foreach my $value(@s) 
			{               
			   print "$value\n";
			   print FH "$value\n"  ;
...
#Defining logo path
my $file = "E:\\Abhi\\mails\\TataMotors.JPEG";
my $file2 = "E:\\Abhi\\mails\\TATA.png";

#PDF paths
my $drw_pdf="$resultDir\\$item.pdf";


my $finalpdf = "$resultDir\\$item-combine.pdf";
my $OutputPDF = "$resultDir\\$item-template.pdf";




#Reading File
open (FILE,"$resultDir\\attributes.txt") or die "Couldn't open file $!";
my(@linearr) = <FILE>;
close(FILE);
my $PartNo_Rev_Seq;
my $Description;
...
foreach my $input_file (@input_files) {
    ...
    foreach my $numpage (@numpages) {
        ...
        # Get page size
		#my @rectangle = $input_pdf->default_page_size();
		#print "Info :: $input_file = Sheet Size of Page No $numpage = @rectangle \n";
...
if (-s $output_file)
{
	print "Info :: Output $output_file File successfuly created\n";
}
else
{
	print "Error :: Output $output_file File Failed to create\n";
}


rename($drw_pdf,"$resultDir\\$item-old.pdf") ||	die ( "Error in renaming" );
rename("$finalpdf",	"$drw_pdf") ||	die ( "Error in renaming" );
